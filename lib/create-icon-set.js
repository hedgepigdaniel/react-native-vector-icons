...
var NativeIconAPI = NativeModules ? (NativeModules.RNVectorIconsManager || NativeModules.RNVectorIconsModule) : undefined;
...
export default function createIconSet(glyphMap, fontFamily, fontFile) {
  ...
  if (Platform.OS === 'android' && fontFile) {
    ...
    const css = `@font-face {
      ...
      src: url('${require(`react-native-vector-icons/Fonts/${fontFile}`)}') format('truetype');
    }`;
